...
- 送料、接近和有限补偿的累计距离不能超过 `toolchange_feed_hard_limit`。
...
- 正常回料和相关恢复请求的累计距离不能超过 `toolchange_retract_hard_limit`。
...
- 送料失败优先暂停打印，不取消打印任务。
- 触及送料或回料绝对硬上限时停止换料、报告失败阶段并暂停打印，绝不调用
  `CANCEL_PRINT`。
...
- `ace_config_version` 标识配置结构；缺失该键的旧配置按兼容模式加载。
- 旧配置缺少独立消抖或绝对硬上限时，由驱动采用兼容默认值或从既有长度派生兼容
  边界，不要求强制替换整个 `ace.cfg`。
...
- 当前开发分支的新配置契约只完成静态部署验证；尚未用送料、回抽、切刀或完整换料
  动作证明本机机械参数。
